...
/*Input data is always shifted in most significant byte and bit first. Output
data is always shifted out most significant
byte first. The ORDER bit only controls the bit order of the output data within
the byte.*/
...
/* MUX - Multiplexer Control Register 0 (see p31 - bring together with bitwise
 * OR | */
/* BIT7  - BIT6  -  BIT5   -  BIT4   -  BIT3   -  BIT2   -  BIT1   -  BIT0 */
...
/*When not using CLKOUT, it is recommended that it be turned off. These bits can
 * only be reset using the RESET pin.*/
...
/*The Sensor Detect Current Sources can be activated to verify the integrity of
an external sensor supplying a signal to the
ADS1255/6. A shorted sensor produces a very small signal while an open-circuit
